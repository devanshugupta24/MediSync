      ...
      <div>
        {appointments.map((item,index)=>(
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
            <div>
              <img className='w-32 bg-[#7badba]' src={item.docData.image} alt="" />
            </div>
            ...
            <div className='flex flex-col gap-2 justify-end'>
              {!item.cancelled && item.payment && !item.isCompleted && <button className='sm:min-w-48 py-2 border rounded text-green-600 bg-green-200'>Paid</button>}
              ...
              {item.cancelled  &&<button className='sm:min-w-48 py-2 border text-sm border-red-500 rounded text-red-500'>Appointment cancelled</button>}
